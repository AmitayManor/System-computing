...
int writeCompanyToBinaryFile(const Company* company, FILE* fp) {
    int nameLen = strlen(company->name) + 1; 
...
void free_company(Company* company) {
    if (company) {
        ...
        for (int i = 0; i < company->numTravels; i++) {
            free_interstellar_travel(company->travels[i]); 
        }
        free(company->travels);
        free(company);
    }
...
SpaceCraft* searchSpaceCraftFromFile(Company* company, const id) {
    ...
    SpaceCraft** found = (SpaceCraft**)bsearch(&searchKeyPtr, company->spaceCrafts, company->numSpacecrafts, sizeof(SpaceCraft*), compareSpaceCraftByID);
